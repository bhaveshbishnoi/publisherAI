  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 shrink-0">
            <button
              ...
              className="flex items-center justify-center gap-2.5 rounded-2xl bg-gradient-to-r from-emerald-600 to-indigo-600 px-6 py-3.5 text-sm font-extrabold text-white shadow-md shadow-emerald-500/20 hover:scale-[1.03] transition-all disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3.5 text-xs font-bold text-slate-700 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-800 transition-all"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3 mb-3 text-slate-900 font-bold text-sm">
            ...
            <ol className="list-decimal list-inside space-y-2.5 text-xs text-slate-600 leading-relaxed">
              ...
              <li>Navigate to your domain&apos;s <code className="text-indigo-700 bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded">public_html</code> or root directory via File Manager.</li>
              <li>Upload and extract <strong className="text-slate-900">{requirements.domain}-bundle.zip</strong>.</li>
              <li>Ensure PHP version is set to <strong className="text-emerald-700">PHP 8.0+</strong> in MultiPHP Manager.</li>
              <li>If using MySQL features, import <code className="text-emerald-700 bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded">database.sql</code> into phpMyAdmin and update credentials in <code className="text-purple-700 bg-purple-50 border border-purple-100 px-1.5 py-0.5 rounded">config/database.php</code>.</li>
            </ol>
          </div>
          ...
          <div className="mt-5 pt-3 border-t border-slate-100">
            <div className="flex items-center justify-between mb-1.5 text-xs text-slate-500 font-mono">
              ...
              <button onClick={handleCopyHtaccess} className="flex items-center gap-1 text-indigo-600 hover:text-indigo-800 font-bold">
                {copiedHtaccess ? <Check className="h-3 w-3 text-emerald-600" /> : <Copy className="h-3 w-3" />}
                <span>{copiedHtaccess ? 'Copied' : 'Copy'}</span>
              </button>
            </div>
            <pre className="rounded-xl border border-slate-800 bg-slate-900 p-3 font-mono text-[11px] text-slate-200 overflow-x-auto max-h-36 shadow-inner">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <pre className="rounded-xl border border-slate-800 bg-slate-900 p-3.5 font-mono text-[11px] text-slate-200 overflow-x-auto shadow-inner">
              <code>{`server {
    listen 80;
    ...
    location ~* \\.(css|js|webp|svg|png)$ {
        ...
        add_header Cache-Control "public, no-transform";
          ...
          <div className="mt-4 rounded-xl border border-indigo-200 bg-indigo-50 p-3.5 flex items-start gap-2.5">
            <ShieldCheck className="h-4 w-4 text-indigo-600 shrink-0 mt-0.5" />
            <p className="text-[11px] text-slate-700 leading-relaxed">
              <strong className="text-indigo-800 font-semibold">SSL Verification Requirement:</strong> Google AdSense mandates valid HTTPS encryption (`Let&apos;s Encrypt` or free Cloudflare SSL) before verifying ad tag ownership.
            </p>
          </div>
        </div>
      </div>
