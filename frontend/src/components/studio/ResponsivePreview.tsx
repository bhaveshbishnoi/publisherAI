...
export default function ResponsivePreview({ files }: ResponsivePreviewProps) {
  ...
  const sandboxHtml = useMemo(() => {
    ...
    if (!pageFile) return '<html><body style="background:#f8fafc;color:#0f172a;font-family:sans-serif;padding:2rem;">No HTML pages generated yet.</body></html>';
  ...
  const viewportStyles = {
    ...
    tablet: 'w-[768px] h-[680px] rounded-[24px] border-8 border-slate-300 shadow-xl',
    mobile: 'w-[375px] h-[680px] rounded-[40px] border-[12px] border-slate-300 shadow-xl',
  };
  ...
  return (
    <div className="flex flex-col items-center space-y-4 animate-in fade-in duration-200">
      ...
      <div className="w-full flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="rounded-xl border border-slate-300 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-900 focus:border-indigo-600 focus:outline-none"
          >
            {htmlPages.map((p) => (
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-xl border border-slate-200 bg-slate-50 p-2 text-slate-600 hover:border-indigo-400 hover:text-indigo-600 transition-all"
          >
      ...
      <div className="w-full rounded-2xl border border-slate-200 bg-slate-100 p-6 flex items-center justify-center overflow-auto shadow-inner min-h-[720px]">
